...
export function RiskInterpretationWorkspace() {
  ...
  return <div className="page interpretation-page">
    <header className="interpretation-commandbar">
      <div><span className="section-kicker"><Sparkles size={13}/> GREENLENS · STRUCTURED INTERPRETATION</span><h2>AI 风险解读</h2><p>机器完成指标解释、证据组织和比较；研究人员负责理解语境与使用结论。</p></div>
      <div className="interpretation-actions">
        <button className="secondary-button" onClick={openOriginal}><FileText size={15}/>查看原文</button>
        <button className="secondary-button" onClick={addToCompare}><GitCompareArrows size={15}/>{company && compareIds.includes(company.companyId) ? "移出对比" : "加入对比"}</button>
        <button className="secondary-button" disabled={!interpretation} onClick={exportSummary}><Download size={15}/>导出摘要</button>
        <button className="quiet-button" onClick={() => setFeedbackOpen(true)}><MessageSquareWarning size={15}/>报告问题</button>
      </div>
    </header>
...
function InterpretationContent({ interpretation, view, selectedCitation, pageReference, onCitation }: { interpretation: RiskInterpretation; view: RiskInterpretationFocus; selectedCitation?: RiskInterpretationCitation; pageReference: EvidencePageReference | null; onCitation: (citation: RiskInterpretationCitation) => void }) {
  ...
  return <>
    ...
    {showEvidence ? <section className="interpretation-section evidence-ledger"><header><div><span className="section-kicker">EVIDENCE LEDGER</span><h4>证据账本</h4></div><small>自动解读 ↔ 可追溯来源</small></header><div className="evidence-ledger-list">{interpretation.drivers.map((driver, index) => {
      const citations = interpretation.citations.filter((item) => driver.citationIds.includes(item.id));
      return <article key={driver.metricCode}><div className="ledger-finding"><span className="ledger-index">{String(index + 1).padStart(2, "0")}</span><div><small>自动解读 · {driver.metricCode}</small><strong>{driver.label}</strong><p>{driver.explanation}</p></div></div><div className="ledger-connector"><i/></div><div className="ledger-sources">{citations.length ? citations.map((citation) => <button key={citation.id} className={selectedCitation?.id === citation.id ? "selected" : ""} onClick={() => onCitation(citation)}><span><Quote size={13}/>{citation.label}<em className={`statement-${citation.kind}`}>{statementLabel(citation.kind)}</em></span><strong>{citation.excerpt}</strong><small>{citation.sourceLabel}{citation.eventDate ? ` · ${citation.eventDate}` : ""}</small></button>) : <div className="ledger-missing"><AlertTriangle size={14}/><span>当前指标没有可定位引用，不能把解释视为已核实事实。</span></div>}</div></article>;
...
function toMarkdown(item: RiskInterpretation) { return [`# ${item.companyName} ${item.reportYear} 年 ESG 风险解读`, "", `> ${item.headline}`, "", item.summary, "", "## 主要风险来源", ...item.drivers.map((driver) => `- ${driver.metricCode} ${driver.label}：${formatPercent(driver.riskValue)}。${driver.explanation}`), "", "## 证据引用", ...(item.citations.length ? item.citations.map((citation, index) => `${index + 1}. ${citation.label}｜${citation.sourceLabel}\n   ${citation.excerpt}`) : ["- 当前没有可引用证据。"]), "", "## 不确定性与证据缺口", ...item.evidenceGaps.map((gap) => `- ${gap}`), `- 不确定性：${uncertaintyLabel(item.uncertainty.level)}（${item.uncertainty.reasons.join("；")}）`, "", "## 比较", `- ${item.history.text}`, `- ${item.industry.text}`, "", "## 建议研究动作", ...item.recommendedActions.map((action) => `- ${action}`), "", `数据版本：${item.versions.data}｜模型：${item.versions.model}｜评分：${item.versions.score}`, "", "风险结果仅用于研究筛查，不构成企业漂绿认定。"].join("\n"); }
