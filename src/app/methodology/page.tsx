...
const methods=[
  ...
  {id:"esgsi",title:"ESI · 环境修辞—内容差异",formula:"sentiment_normalized - environmental_information_normalized",direction:"越高表示积极环境语气与实质环境信息差距越大",threshold:"阈值由风险规则版本返回",example:"同时保留可为负数的 rawValue 与 0–1 normalizedValue。",status:"输入与归一化均可追溯"},
  {id:"eaa",title:"EAA-ESI · 最终调整指数",formula:"ESI_norm + λ₁(1-EASS) + λ₂IR + λ₃UPR",direction:"越高表示综合风险信号越强",threshold:"基础相对风险 + 红旗升级，由后端返回",example:"前端分别展示 finalRaw 与 finalNormalized，不重新分类。",status:"λ、归一化和分类规则版本化"},
...
export default function MethodologyPage(){
  ...
  return<div className="methodology-layout"><aside className="method-toc"><span>本页目录</span>{["函数链","指标字典","数据契约","风险分级","适用边界","版本"].map((item,i)=><a href={`#section-${i}`} key={item}>{item}</a>)}</aside><article className="method-document"><header><span className="demo-badge">METRIC CONTRACT V2</span><h2>方法与模型</h2><p>从报告处理、环境议题与行动证据到最终风险分级的可追溯函数链。AI 负责解释与引用，风险不构成企业漂绿认定。</p></header>
    ...
    <section id="section-5"><span className="section-kicker">版本</span><h3>当前演示契约</h3><table><tbody><tr><th>Schema</th><td><code>metric-contract-v2</code></td></tr><tr><th>模型</th><td><code>EAA-ESI-DEMO-2.0</code></td></tr><tr><th>状态</th><td>全部企业、报告、财务与事件数值为合成 Mock</td></tr></tbody></table></section>
