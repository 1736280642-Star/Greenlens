...
import { AlertTriangle, Bot, CalendarDays, ChevronRight, Download, ExternalLink, FileText, GitCompareArrows, MoreHorizontal, RefreshCw, Search, ShieldAlert, Sparkles, X } from "lucide-react";
...
export default function CompanyDetailPage() {
  ...
  function openInterpretation(evidence?: string, feedback = false) {
    const query = new URLSearchParams({ companyId: id, year: String(reportYear), view: evidence ? "evidence" : "overview" });
    if (evidence) query.set("evidence", evidence);
    if (feedback) query.set("feedback", "open");
  ...
  return <div className="page company-detail-page">
    <header className="company-header"><div className="breadcrumbs"><Link href="/companies">企业</Link><ChevronRight size={13}/><span>{company.companyName}</span></div><div className="company-title-row"><div><div className="company-title"><h2>{company.companyName}</h2><span className="demo-badge">LIVE DATA</span></div><p><code>{company.stockCode}</code>   {company.industry}   报告年度 {company.reportYear}</p></div><div className="header-actions"><button className="secondary-button" onClick={() => { if (!toggleCompare(company.companyId)) showToast("最多同时比较 5 家"); }}><GitCompareArrows size={15}/>{compareIds.includes(company.companyId) ? "移出对比" : "加入对比"}</button><button className="secondary-button" onClick={exportSummary}><Download size={15}/>导出摘要</button><button className="primary-button" onClick={() => openInterpretation(currentEvidence?.id ?? getMetric(company,"UPR")?.evidenceIds[0])}><Sparkles size={15}/>打开 AI 解读</button><button className="icon-button" title="复制分析链接" aria-label="复制分析链接" onClick={() => { navigator.clipboard.writeText(location.href); showToast("分析链接已复制"); }}><MoreHorizontal/></button></div></div><div className="company-meta"><span>报告发布日 <code>{company.publishDate}</code></span><span>数据版本 <code>{company.versions.data}</code></span><span>评分版本 <code>{company.versions.model}</code></span><span>证据状态 <code>{company.evidenceStatus}</code></span></div></header>
    <section className="risk-summary-band"><button><span>EAA-ESI</span><strong className="risk-score">{formatPercent(company.finalIndex)}</strong><em>{company.riskBand==="high"?"高风险，建议优先解读":company.riskBand==="medium"?"中风险，建议核验":company.riskBand==="low"?"低风险信号":"暂不可评分，需补充输入"}</em></button><button><span>EASS</span><strong className="risk-score">{formatMetricPercent(company,"EASS")}</strong><em>越高表示行动越实质</em></button><button><span>IR / UPR</span><strong>{formatMetricPercent(company,"IR")} / {formatMetricPercent(company,"UPR")}</strong><em>模糊声明 / 未验证计划</em></button><button onClick={() => { const evidenceId=getMetric(company,"UPR")?.evidenceIds[0]; if(evidenceId) openEvidence(evidenceId); }}><span>主要原因</span><strong>未验证计划比例</strong><em>定位分子与证据 →</em></button></section>
    ...
    {tab === "evidence" && <LiveEvidenceView items={items} current={currentEvidence} onSelect={(item) => { selectEvidence(item.id); setTab("evidence", item.id); }} onAI={() => openInterpretation(currentEvidence?.id)} onReview={() => openInterpretation(currentEvidence?.id, true)} onPdf={(item) => void openPdf(item)} />}
    {tab === "facts" && <FactsView events={events} evidenceId={items.find((item) => item.type === "external")?.id} onReview={(evidence) => openInterpretation(evidence, true)} />}
...
function LiveEvidenceView({ items, current, onSelect, onAI, onReview, onPdf }: { items: EvidenceItem[]; current?: EvidenceItem; onSelect: (item: EvidenceItem) => void; onAI: () => void; onReview: () => void; onPdf: (item: EvidenceItem) => void }) {
  ...
  return <div className="evidence-workspace">
    ...
    <aside className="evidence-explanation"><span className={`status-chip ${current.status}`}>{current.status === "insufficient" ? "证据不足" : "证据已关联"}</span><h3>{current.title}</h3><p>原文段落由后端从 PDF 文本层提取并持久化；「查看原 PDF」仅返回证据定位页的只读文本，不下发文件本体。</p><dl><div><dt>来源文件</dt><dd>{current.sourceLabel}</dd></div><div><dt>定位页码</dt><dd>{current.page ?? "--"}</dd></div><div><dt>结构化分类</dt><dd>{current.actionClass ?? current.type}</dd></div></dl><div className="inline-actions"><button className="secondary-button" onClick={onReview}>报告问题</button><button className="primary-button" onClick={onAI}><Bot size={15}/>打开 AI 解读</button></div></aside>
  </div>;
}
...
function FactsView({ events, evidenceId, onReview }: { events: ViolationEvent[]; evidenceId?: string; onReview: (id: string) => void }) {
  ...
  return <div className="facts-layout"><section className="panel event-timeline"><header className="panel-header"><h3>违规与监管事件</h3><span>违规年度 ≠ 公告日期</span></header>{events.map((event,index)=><button className={selectedIndex===index?"selected":""} onClick={()=>setSelected(index)} key={event.id}><span><CalendarDays size={15}/>{event.announcementDate}</span><strong>{event.title ?? event.violationTypes.join("、")}</strong><small>{event.authority ?? event.sourceLabel}</small></button>)}</section><section className="panel fact-detail"><header className="panel-header"><div><h3>{current.title ?? current.violationTypes.join("、")}</h3><p>{current.announcementDate}   {current.authority ?? current.sourceLabel}</p></div><span className={`status-chip ${current.reviewStatus}`}>{current.reviewStatus === "verified" ? "已核验" : current.reviewStatus === "insufficient" ? "证据不足" : "待确认关联"}</span></header><div className="panel-body"><p>{current.behavior}</p><div className="fact-metrics"><div><span>关联主体</span><strong>{current.relation ?? "待确认"}</strong><small>{current.subjectName ?? "主体名称缺失"}</small></div><div><span>违规年度</span><strong>{current.violationYears.join(" / ")}</strong><small>{current.violationTypes.join("、")}</small></div><div><span>处罚金额</span><strong>{current.companyPenalty == null ? "未披露" : `¥${current.companyPenalty.toLocaleString()}`}</strong><small>{current.action}</small></div></div><div className="inline-actions"><button className="secondary-button" disabled={!evidenceId} onClick={()=>evidenceId&&onReview(evidenceId)}>报告关联问题</button></div>{current.sourceUrl&&<a href={current.sourceUrl} onClick={(event)=>{event.preventDefault();showToast("虚构来源不发起外部访问");}} className="source-link">查看虚构来源 <ExternalLink size={13}/></a>}</div></section></div>;
}
function HistoryView({ company, history }: { company: CompanyYearRecord; history: CompanyMetricHistoryPoint[] }) {
  const options: Array<[string, MetricCode | "FINAL"]> = [["EAA-ESI","FINAL"],["EASS","EASS"],["IR","IR"],["UPR","UPR"],["ESI","ESGSI"],["Imbalance","IMBALANCE"]];
